...
def MNIST(batch_size):
    ...
    test_data = gluon.data.DataLoader(gluon.data.vision.MNIST(root="MNIST", train = False , transform = transform) ,10000 , shuffle=True) #Loads data from a dataset and returns mini-batches of data.
...
def FashionMNIST(batch_size):
    ...
    test_data = gluon.data.DataLoader(gluon.data.vision.FashionMNIST(root="FashionMNIST", train = False, transform=transform) , 10000 , shuffle=True) #Loads data from a dataset and returns mini-batches of data.
...
def CNN_Autoencoder(epoch = 100 , batch_size=10, save_period=10 , load_period=100 , weight_decay=0.001 ,learning_rate= 0.1 , dataset = "FashionMNIST", ctx=mx.gpu(0)):
    ...
    if os.path.exists(path):
        ...
        W1 = nd.random_normal(loc=0 , scale=0.01 , shape=(60,1,3,3) , ctx=ctx)
        B1 = nd.random_normal(loc=0 , scale=0.01 , shape=60 , ctx=ctx)

        W2 = nd.random_normal(loc=0 , scale=0.01 , shape=(30,60,3,3) , ctx=ctx)
        B2 = nd.random_normal(loc=0 , scale=0.01 , shape=30 , ctx=ctx)

        W3 = nd.random_normal(loc=0 , scale=0.01 , shape=(15,30,3,3) , ctx=ctx)
        B3 = nd.random_normal(loc=0 , scale=0.01 , shape=15 , ctx=ctx)

        #######################################################################

        W4 = nd.random_normal(loc=0 , scale=0.01 , shape=(10,15,3,3) , ctx=ctx)
        B4 = nd.random_normal(loc=0 , scale=0.01 , shape=10 , ctx=ctx)

        #######################################################################

        W5 = nd.random_normal(loc=0 , scale=0.01 , shape=(15,10,3,3) , ctx=ctx)
        B5 = nd.random_normal(loc=0 , scale=0.01 , shape=15 , ctx=ctx)

        W6 = nd.random_normal(loc=0 , scale=0.01 , shape=(30,15,3,3) , ctx=ctx)
        B6 = nd.random_normal(loc=0 , scale=0.01 , shape=30 , ctx=ctx)

        W7 = nd.random_normal(loc=0 , scale=0.01 , shape=(60,30,3,3) , ctx=ctx)
        B7 = nd.random_normal(loc=0 , scale=0.01 , shape=60 , ctx=ctx)

        W8 = nd.random_normal(loc=0 , scale=0.01 , shape=(1,60,3,3) , ctx=ctx)
        B8 = nd.random_normal(loc=0 , scale=0.01 , shape=1 , ctx=ctx)
