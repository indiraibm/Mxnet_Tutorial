...
def FNN(epoch = 100 , batch_size=10, save_period=10 , load_period=100 , weight_decay=0.001 ,learning_rate= 0.1 , dataset = "MNIST", ctx=mx.gpu(0)):
    ...
    if os.path.exists(path):
        ...
        W1 = nd.random_normal(loc=0, scale=0.01, shape=(num_inputs, num_hidden),ctx=ctx)
        B1 = nd.random_normal(loc=0, scale=0.01, shape=num_hidden,ctx=ctx)
        W2 = nd.random_normal(loc=0, scale=0.01, shape=(num_hidden, num_outputs),ctx=ctx)
        B2 = nd.random_normal(loc=0, scale=0.01, shape=num_outputs,ctx=ctx)
        params = [W1 , B1 , W2 , B2]
