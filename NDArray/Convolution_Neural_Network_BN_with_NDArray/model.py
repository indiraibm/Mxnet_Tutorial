...
def CNN(epoch = 100 , batch_size=256, save_period=10 , load_period=100 , weight_decay=0.001 ,learning_rate= 0.1 , dataset = "MNIST", ctx=mx.cpu(0)):
    ...
    if os.path.exists(path1):
        ...
        print(MOVING_VARS)
        MOVING_MEANS=MOVING_MEANS.as_in_context(ctx)
        MOVING_VARS=MOVING_VARS.as_in_context(ctx)
        ...
        W1 = nd.random_normal(loc=0 , scale=weight_scale , shape=(60,color,3,3) , ctx=ctx)
        B1 = nd.random_normal(loc=0 , scale=weight_scale , shape=60 , ctx=ctx)

        gamma1 = nd.random_normal(shape=60, loc=1, scale=BN_weight_scale, ctx=ctx)
        beta1 = nd.random_normal(shape=60, scale=BN_weight_scale, ctx=ctx)

        W2 = nd.random_normal(loc=0 , scale=weight_scale , shape=(30,60,6,6) , ctx=ctx)
        B2 = nd.random_normal(loc=0 , scale=weight_scale , shape=30 , ctx=ctx)

        gamma2 = nd.random_normal(shape=30, loc=1, scale=BN_weight_scale, ctx=ctx)
        beta2 = nd.random_normal(shape=30, scale=BN_weight_scale, ctx=ctx)
        ...
        W3 = nd.random_normal(loc=0 , scale=weight_scale , shape=(120, reshape) , ctx=ctx)
        B3 = nd.random_normal(loc=0 , scale=weight_scale , shape=120 , ctx=ctx)

        gamma3 = nd.random_normal(shape=120, loc=1, scale=BN_weight_scale, ctx=ctx)
        beta3 = nd.random_normal(shape=120, scale=BN_weight_scale, ctx=ctx)

        W4 = nd.random_normal(loc=0 , scale=weight_scale , shape=(64, 120) , ctx=ctx)
        B4 = nd.random_normal(loc=0 , scale=weight_scale , shape=64 , ctx=ctx)

        gamma4 = nd.random_normal(shape=64, loc=1, scale=BN_weight_scale, ctx=ctx)
        beta4 = nd.random_normal(shape=64, scale=BN_weight_scale, ctx=ctx)

        W5 = nd.random_normal(loc=0 , scale=weight_scale , shape=(num_outputs , 64) , ctx=ctx)
        B5 = nd.random_normal(loc=0 , scale=weight_scale , shape=num_outputs , ctx=ctx)
    ...
    def network(X, is_training=True, drop_rate=0.0): # formula : output_size=((input−weights+2*Padding)/Stride)+1
        ...
        '''
        F_H1 =nd.Activation(BN(nd.FullyConnected(data=P_H2 , weight=W3 , bias=B3 , num_hidden=120), gamma3, beta3 ,scope_name="BN3",is_training=is_training),act_type="sigmoid")
        F_H1 =nd.Dropout(data=F_H1, p=drop_rate)
        F_H2 =nd.Activation(BN(nd.FullyConnected(data=F_H1 , weight=W4 , bias=B4 , num_hidden=64), gamma4, beta4, scope_name="BN4",is_training=is_training),act_type="sigmoid")
