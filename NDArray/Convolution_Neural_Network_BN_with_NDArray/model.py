...
def CIFAR10(batch_size):
    ...
    test_data = gluon.data.DataLoader(gluon.data.vision.CIFAR10(root="CIFAR10", train = False, transform=transform) , 100 , shuffle=False) #Loads data from a dataset and returns mini-batches of data.=128
...
def CNN(epoch = 100 , batch_size=256, save_period=10 , load_period=100 , weight_decay=0.001 ,learning_rate= 0.1 , dataset = "MNIST", ctx=mx.cpu(0)):
    ...
    if os.path.exists(path):
        ...
        ma1=nd.zeros(1,ctx=ctx)
        mv1=nd.zeros(1,ctx=ctx)
        ...
        ma2=nd.zeros(1,ctx=ctx)
        mv2=nd.zeros(1,ctx=ctx)
        ...
        ma3=nd.zeros(1,ctx=ctx)
        mv3=nd.zeros(1,ctx=ctx)
        ...
        ma4=nd.zeros(1,ctx=ctx)
        mv4=nd.zeros(1,ctx=ctx)
